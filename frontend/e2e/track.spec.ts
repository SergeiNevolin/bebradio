...
test('searches for a track, adds it, and verifies it plays', async ({ page }) => {
  const id = uniqueId()
  ...
  await expect(page.getByRole('heading', { name: /Queue/ })).not.toHaveText(/Queue \(0\)/, { timeout: 120000 })

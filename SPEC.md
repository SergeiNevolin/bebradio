...
1. **Загрузка.** `YouTubeProvider.download()` дополнительно передаёт `yt-dlp`:
   `--write-subs --write-auto-subs --sub-langs "ru.*,en.*" --sub-format vtt/best
   --convert-subs vtt`. Рядом с `media_<hash>.m4a` появляется `media_<hash>.<lang>.vtt`
   (напр. `media_<hash>.en.vtt`). Отсутствие субтитров — не ошибка загрузки.
...
3. **Отдача.** Новый эндпоинт media-service `GET /v1/media/{media_id}/captions?lang=`:
   находит `.vtt` по `media_id`, парсит существующим `_parse_vtt`, возвращает
   `{"lang": str, "auto": bool, "cues": [{start, dur, text}]}`. Нет `.vtt` →
   `{"lang": "", "auto": false, "cues": []}` со статусом 200.
...
- **Синхронизация.** Таймкоды авто-VTT абсолютны от начала видео; `bestaudio`
  начинается с 0 той же точки — должно совпасть. Проверить на реальном треке
  (клип с интро, где видео ≠ аудио по длине).
